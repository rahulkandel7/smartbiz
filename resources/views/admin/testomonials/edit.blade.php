    ...
    <div class="flex-1">
        <div class="px-5 py-5">
            <h3 class="text-primary text-3xl text-[#284b81] font-bold">
                Update testomonial Members:- {{$testomonial->name}}
            </h3>
            ...
            <form method="post" action="{{route('admin.testomonial.update',$testomonial->id)}}" enctype="multipart/form-data">
                @method('put')
                @csrf
                {{-- For Name --}}
                <div class="mt-5">
                    <label for="name" class="block font-bold text-gray-600 dark:text-gray-200">Name</label>
                    <input type="text" name="name" id="name" class="border-[#5976e970] border-2 rounded-md w-full focus:ring-transparent focus:border-[#284b81] bg-transparent mt-2" required value="{{$testomonial->name}}">
                </div>

                {{-- For Post --}}
                <div class="mt-5">
                    <label for="post" class="block font-bold text-gray-600 dark:text-gray-200">Post</label>
                    <input type="text" name="post" id="post" class="border-[#5976e970] border-2 rounded-md w-full focus:ring-transparent focus:border-[#284b81] bg-transparent mt-2" required value="{{$testomonial->post}}">
                </div>
                ...
                <div class="mt-5">
                    ...
                    <textarea name="description" id="description" class="border-[#5976e970] border-2 rounded-md w-full focus:ring-transparent focus:border-[#284b81] bg-transparent mt-2" required>{{$testomonial->description}}</textarea>
                </div>
