...
@section('main')
    ...
    <div class="flex-1">
        <div class="px-5 py-5">
            <h3 class="text-[#284b81] text-4xl font-bold">
                Messages
            </h3>
            <hr>
                ...
                <div class="w-44 h-32 mx-4 mt-4 p-2 shadow-md dark:shadow-emerald-800 rounded-md bg-emerald-500 shadow-emerald-200 hover:shadow-sm hover:cursor-pointer">
                    <h2 class="text-xl font-bold text-white">No of Messages</h2>
                    <div class="flex justify-between">
                        <i class="fas fa-comment fa-2x text-white opacity-70"></i>
                    <h1 class="text-4xl font-bold text-right text-white mr-4">{{count($messages)}}</h1>
                    </div>
                </div>
                ...
                @foreach ($messages as $message)
                    <tr>
                        ...
                        <td class="text-gray-600 font-semibold px-5 border w-32 py-2">
                            ...
                            <button onclick="showdelete({{$message->id}})">
            ...
            <div id="deletemodal"  class="white-card hidden dark:bg-white dark:bg-opacity-10 overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-20 z-50 justify-center items-center md:h-full md:inset-0  align-middle"  >
                <div class="relative px-4 w-full max-w-md h-full md:h-auto mx-auto ">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <form class="px-6 pb-4 space-y-6 lg:px-8 sm:pb-6 xl:pb-8" method="POST" action="{{route('contacts.delete')}}">
                        @csrf
            ...
            <div class="fixed bottom-3 right-7">
                <a href="{{ route('admin.testomonial.create') }}">
                    <div class="bg-[#284b81] w-14 h-14 rounded-full bg-primary text-white text-lg cursor-pointer">
